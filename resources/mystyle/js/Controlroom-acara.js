...
function openURL(o){
	window.open($(o).attr('data-target-url'),"_blank");
}
function editFileKeterangan(xx,cc){	
	...
	var xxValue = cc.childNodes[3].innerHTML;
	openModalForm("<span style='color : black'>Masukan keterangan informasi tentang file</span><br><input id='temp-file-data-keterangan-id' type='text' value='' placeholder='keterangan'>",function(aa){
		openLoadingBar('refresh list file ...');
		$("#close-form-modal").attr("disabled","true");
		$("#submit-edit-acara-default").attr("disabled","true");
		j("#setAjax").setAjax({
			methode : "POST",
			url : base_url+"Controlfileupload/updateRecord",
			bool : true,
	...
	$('#temp-file-data-keterangan-id').val("");
	console.log(xxValue);
	for(icg = 0; icg<100; icg++)
		xxValue = xxValue.replace('\t',"");
	$('#temp-file-data-keterangan-id').val(xxValue);
}
var hapusIDFile = "";
function hapusFile(xx,cc){
	...
	modalStaticMultipleButton("Anda yakin ingin menghapus file ini ? ",function(){
		...
		j("#setAjax").setAjax({
			methode : "POST",
			url : base_url+"Controlfileupload/removeRecord",
			bool : true,
...
function refreshFileList(){
	...
	j("#setAjax").setAjax({
		methode : "POST",
		url : base_url+"Controlfileupload/getListRecord",
		bool : true,
...
function refreshAcaraDefault(){
	...
	j('#setAjax').setAjax({
		methode : "POST",
		url : base_url+"Controlacarakoor/getTableAcara",
		bool : true,
		content : "kode=JASERVCONTROL",
		sucOk : function(a){
			if(a[0] == '1'){
...
function refreshAcaraNonDefault(){
	...
	j('#setAjax').setAjax({
		methode : "POST",
		url : base_url+"Controlacarakoor/getTableAcaraNonDefault",
		bool : true,
...
function showMeThisRegistrasiContent(a,b){
	...
	j("#setAjax").setAjax({
		methode : 'POST',
		bool : true,
		url : base_url+"Controlacarakoor/getJsonDataRegistrasi",
				...
				message =
					'<div class="content controls">'+
						'<div class="form-row">'+
							...
							'<div class="col-md-9">'+
								'<input value="'+tempssss.judul+'" disabled="">'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							...
							'<div class="col-md-9" style="'+
								...
								'text-align: justify;'+
							'">'+
								'<textarea style="word-break : break-word; text-align : justify; margin: 0px -8.5px 0px 0px; height: 250px; width: 100%;resize:none;" disabled="">'+tempssss.isi+'</textarea>'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							'<div class="col-md-3">Tahun akademik</div>'+
							'<div class="col-md-9">'+
								'<input disabled="" value="'+tempssss.year+"-"+(parseInt(tempssss.year)+1)+" Semester "+(tempssss.semester == '1'?"Gasal":"Genap")+'">'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							'<div class="col-md-3">Waktu pelaksanaan</div>'+
							'<div class="col-md-9">'+
								'<input disabled="" value="'+tempssss.start+" hingga "+tempssss.end+'">'+
							'</div>'+
						'</div>'+
					'</div>';
				;
...
function showMeThisRegistrasiContentNonDefault(a){
	...
	j("#setAjax").setAjax({
		methode : 'POST',
		bool : true,
		url : base_url+"Controlacarakoor/getJsonDataRegistrasiNonDefault",
				...
				message =
					'<div class="content controls">'+
						'<div class="form-row">'+
							'<div class="col-md-3">Judul</div>'+
							'<div class="col-md-9">'+
								'<input value="'+tempssss.data[4]+'" disabled="">'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							'<div class="col-md-3">Isi</div>'+
							'<div class="col-md-9" style="'+
								...
								'text-align: justify;'+
							'">'+
								'<textarea style="word-break : break-word; text-align : justify; margin: 0px -8.5px 0px 0px; height: 250px; width: 100%;resize:none;" disabled="">'+tempssss.data[5]+'</textarea>'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							'<div class="col-md-3">Terakhir dirubah</div>'+
							'<div class="col-md-9">'+
								'<input disabled="" value="'+tempssss.data[2]+'">'+
							'</div>'+
						'</div>'+
					'</div>';
				;
					...
					'<div class="content controls">'+
						...
						'<div class="form-row">'+
							...
							'<div class="col-md-9" style="'+
								...
								'text-align: justify;'+
							'">'+
								'<textarea style="text-align : justify; margin: 0px -8.5px 0px 0px; height: 250px; width: 100%;resize:none;" disabled="">Tidak tersedia</textarea>'+
							'</div>'+
						'</div>'+
						'<div class="form-row">'+
							'<div class="col-md-3">Terakhir dirubah</div>'+
							'<div class="col-md-9">'+
								'<input disabled="" value="Tidak tersedia">'+
							'</div>'+
						'</div>'+
					'</div>';
				;
...
function addNewEvent(){
			...
			setTimeout(function(){
				...
				if(error == 0){
					...
					j('#setAjax').setAjax({
						methode : "POST",
						bool : true,
						url : base_url+"Controlacarakoor/setNewEvent",
						content : "title="+title+
						"&summary="+summary+
	...
	$('#title-event-new').on('change',function(){
		...
		if($(this).val() == ""){
			...
			j("#setAjax").setAjax({
				methode : "POST",
				url : base_url+"Controlacarakoor/getCheck",
				bool : true,
...
function editEventAktif(ccc){
			...
			setTimeout(function(){
				...
				if(error == 0){
					...
					j('#setAjax').setAjax({
						methode : "POST",
						bool : true,
						url : base_url+"Controlacarakoor/setDataEditEvent",
						content : 
	...
	j("#setAjax").setAjax({
		methode : "POST",
		bool : true,
		url : base_url+"Controlacarakoor/getJsonDataEventActive",
...
function editAkademikAktif(){
			...
			setTimeout(function(){
				...
				if(error == 0){
					...
					j('#setAjax').setAjax({
						methode : "POST",
						bool : true,
						url : base_url+"Controlacarakoor/setNewAkademik",
						content : 
	...
	j("#setAjax").setAjax({
		methode : "POST",
		bool : true,
		url : base_url+"Controlacarakoor/getJsonDataRegistrasiActive",
		content : "kode=JASERVCONTROL",
		sucOk : function(a){
			//alert(a);
			//var bb = JSON.parse(a);
