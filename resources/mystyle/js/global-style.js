...
}function openModalContactWide(contact,header){
	j('#mdc-message-wide').setInHtml(header);
	j('#content-contact-wide').setInHtml(contact);
	$('#modal_default_contact_wide').modal({backdrop : 'static'});
}
...
$('document').ready(function(){
	...
	addModalToComponent(
			...
			'<div class="modal" id="modal_default" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">'+
				...
				'</div>'+
			'</div>'+
			'<div class="modal modal-wide" id="modal_default_contact_wide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">'+
				'<div class="modal-dialog" style="width :  100%;"> <div class="modal-content">'+
					'<div class="modal-header">'+
						'<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>'+
							'<h4 class="modal-title" id="mdc-message-wide">Contacts</h4>'+
						'</div>'+
						'<div class="modal-body clearfix np">'+
				...
				'</div>'+
			'</div>'+
			'<div id="temp-data-flow-right" style="position : fixed; width : 500px; max-height : 500px; min-height : 200px; display : none; top : 50px; left : 0; background-color : rgba(255,255,255,0.95);">'+
			'</div>'
	);
});
var FlowHovering = {
	show : function(data){
		$("#temp-data-flow-right").html("").html(data).fadeIn('fast');
	},
	hide : function(){
		$("#temp-data-flow-right").fadeOut('fast');
	}
}