...
function reloadChartSeminarAll(activeCanvas){
	if(activeCanvas != activecanvasnow){
		...
		switch(activeCanvas){
			case 1 :
			reloadChartSeminar("canvas1","getJsonTableNow","controller-diagram-1","Diagram Sebagai Dosen Pembimbing", "Dosen Pembimbing");
			break;
			case 2 :
			reloadChartSeminar("canvas2","getJsonTableTesterS","controller-diagram-2","Diagram Sebagai Penguji 1", "Sebagai Penguji 1");
			break;
			case 3 :
			reloadChartSeminar("canvas3","getJsonTableTesterD","controller-diagram-3","Diagram Sebagai Penguji 2", "Sebagai Penguji 2");
			break;
			case 4 :
			reloadChartSeminar("canvas4","getJsonTableTesterT","controller-diagram-4","Diagram Sebagai Penguji 3", "Sebagai Penguji 3");
			break;
		}	
...
function reloadChartSeminar(id,url,dia,title,yAxisText){
	$('#'+dia).html("");
	...
	j('#setAjax').setAjax({
		methode : 'POST',
		url : base_url+'Controlresultseminar/'+url+'',
		bool : true,
		...
		sucOk : function(a){
			if(a[0]=='1'){
				...
				chacheChartVariabelGlobal.title.text = title;
				chacheChartVariabelGlobal.xAxis.categories = temm.data[0];
				chacheChartVariabelGlobal.yAxis.title.text = yAxisText;
				chacheChartVariabelGlobal.series = [{
					name : yAxisText,
					data : temm.data[1]
				}];
				Highcharts.chart(dia,chacheChartVariabelGlobal);
			}else{
...
function showImageTA1(namafile,md5source){
	data="<div style='max-width:100%; text-align:center; margin-left : auto; margin-right:auto;'><img style='max-width : 100%;' src='"+base_url+"Filesupport/"+md5source+"/"+namafile+".jsp'></div>";
...
function showPdfTA1(namafile,md5source){
	...
	'<object type="application/pdf" data="'+base_url+"Filesupport/"+md5source+"/"+namafile+".jsp"+'" style="width : 100%; height : 540px" ><p>Browser anda tidak support pdf</p></object>'+
	'</div>';
	modalStaticSingleInformation("Melihat PDF",data);
}
function showPdfTA1_hover(namafile,md5source){
	data = '<div id="pdf-preview" style="max-width : 100%; text-align : center; margin-left : auto; margin-right:auto;">'+
	'<object type="application/pdf" data="'+base_url+"Filesupport/"+md5source+"/"+namafile+".jsp"+'" style="width : 100%; height : 540px" ><p>Browser anda tidak support pdf</p></object>'+
	'</div>';
	if($(window).width() < 700){
		openLoadingBar("Sentuhan hanya berlaku pada layar dengan resolusi minimum 700px, silahkan klik ...");
		setTimeout(function(){
			closeLoadingBar();
		},2000);
		return;
	}
	FlowHovering.show(data);
}
function showImageTA1_hover(namafile,md5source){
	data="<div style='max-width:100%; text-align:center; margin-left : auto; margin-right:auto;'><img style='max-width : 100%;' src='"+base_url+"Filesupport/"+md5source+"/"+namafile+".jsp'></div>";
	if($(window).width() < 700){
		openLoadingBar("Sentuhan hanya berlaku pada layar dengan resolusi minimum 700px, silahkan klik ...");
		setTimeout(function(){
...
function seminarTA1Agreement(a,b){
	...
	j("#setAjax").setAjax({
		methode : "POST",
		url : base_url+"Controlresultseminar/setNewStatusSeminarTA",
...
function approveThisPenguji(a,b){
	...
	j("#setAjax").setAjax({
		methode : "POST",
		url : base_url+"Controlresultseminar/setStatusProsesTester",
...
function changePenguji1(a,b){
	...
	j("#setAjax").setAjax({
		url : base_url+"Controlresultseminar/setNewPenguji",
		bool : true,
		methode : "POST",
...
function refreshTableSeminarTA1(){
	...
	openLoadingBar("kontak server ...");
	j("#setAjax").setAjax({
		url : base_url+"Controlresultseminar/getTableSeminarTA1",
		bool : true,
		methode : "POST",
...
function refreshTableSeminarTA2(){
	...
	openLoadingBar("kontak server ...");
	j("#setAjax").setAjax({
		url : base_url+"Controlresultseminar/getTableSeminarTA2",
		bool : true,
		methode : "POST",
		...
		sucOk : function(a){
			if(a[0] == '1'){
				$("#tabel-pemerataan-seminar-ta2").fadeOut('slow');
				$("#tabel-pemerataan-seminar-ta2").html("");
				setLoadingBarMessage('proses respon server ...');
				$("#tabel-pemerataan-seminar-ta2").html(a.substr(1,a.length-1));
				if(tabControl.sidang.aksiPenguji) $('.tab-aksi-penguji').fadeOut('fast');
				if(tabControl.sidang.statusKelengkapan) $('.tab-status-kelengkapan').fadeOut('fast');
				if(tabControl.sidang.aksi) $('.tab-aksi').fadeOut('fast');
				if(tabControl.sidang.nilai) $('.tab-nilai').fadeOut('fast');
				$("#tabel-pemerataan-seminar-ta2").fadeIn('slow');
			}
			setTimeout(function(){
				closeLoadingBar();
			},750);
...
function giviTA1Seminar(nim, value){
	...
	j("#setAjax").setAjax({
		url : base_url+"Controlresultseminar/setValueSeminarTA1",
		bool : true,
		methode : "POST",
...
function giviTA2Seminar(nim, value){
	openLoadingBar("Mencoba merubah nilai ...");
	j("#setAjax").setAjax({
		url : base_url+"Controlresultseminar/setValueSeminarTA2",
		bool : true,
		methode : "POST",
