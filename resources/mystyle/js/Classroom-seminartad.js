	...
	$('#seminar1form').submit(function(){
		iframe = $('#frame-layout').load(function (){
			...
			if(returnResponse[0] ==  "="){
				...
				if(parseInt(returnResponse[1]) == 3)
					modalStaticSingleWarning("Terjadi perubahan status, klik disini (<span><a onclick='refreshPageSeminarTad()' style='cursor:pointer;'><i class='icon-refresh' title='refresh table'></i></a></span>)");
				else
...
function refreshPageSeminarTad(){
...
function getDataOnForCalendar(){
	...
	j("#setAjax").setAjax({
		methode : "GET",
		bool : true,
		url :base_url+"Classseminartad/getJSONDataSeminarTA2",
		content : "",
		sucOk : function(a){
			if(a[0] ==  "="){
				window.location = base_url+"Gateinout";
		...
		CalenderOnSemTA2[data.id] = $("#"+data.id).fullCalendar({
			...
			select:function(e,t,n){
				...
				modalStaticBodyMultipleButton("Masukan Jam Seminar anda",templateCalendarTAS,function(finalis){
					...
					j("#ajax").setAjax({
						methode: "POST",
						url:base_url+"Classseminartad/getCheck",
						bool : true,
