...
function addNewDosen(){
			...
			setTimeout(function(){
				...
				if(error == 0){
					j('#setAjax').setAjax({
						methode : "POST",
						bool : true,
						url : base_url+"Controldosens/addNewDosen",
						content : 
	...
	$('#add-nama-dosen').on('change',function(){
		...
		if($(this).val() == ""){
			...
			j("#setAjax").setAjax({
				methode : "POST",
				url : base_url+"Controldosens/getCheck",
				bool : true,
...
function showListMahasiswaAmpuan(g){
	...
	{
		methode : 'POST',
		url : base_url+'Controldosens/getJsonListMahasiswa',
		bool : true,
		content : "nip="+g,
		sucOk : function(a){
			var message="";
...
function refreshTable(page){
	...
	j("#setajax").setAjax({
		methode : 'POST',
		url : base_url+'Controldosens/getTableDosen',
		bool : true,
