...
class Inputjaservfilter {
	...
	*/
	public function stringFiltering($variabel){
		return htmlentities($variabel, ENT_QUOTES);
	}
	public function stringFilteringDecode($variabel){
		return html_entity_decode($variabel, ENT_QUOTES);
	}
	/*
	test : true
	...
	*/
	public function userUnicCodeFiltering($value=null){
		if(($value=="") || ($value == null)){
			return array(false, "nilai tidak boleh kosong");
		}
		if(!preg_match("/^[A-Z0-9-]*$/",$value))
			return array(false,"userniccode mengandung karakter yang tidak diizinkan");
		if(!strpos($value, "-"))
			return array(false, "Userniccode tidak valid");
		if(!$this->isContainAlphabetUpper($value))
			return array(false, "Userniccode tidak valid");
		return array(true, "valid");
	}
	public function nameFiltering($name){
		$name.="";
		$name=$this->stringFiltering($name);
		if($name == "" || $name == " ")
			return (array(false,"tidak boleh kosong"));
		if($this->isContainMoreSpace($name))
	...
	*/
	...
	output
		...
		tidak valid 1 -> "pesan eror"
	*/
	public function passwordFilteringSecure1($password){
		$password.="";
	...
	public function passwordFilteringSecure2($password){
		$password.="";
		if($password == ""||$password==" ")
			return (array(false,"password tidak boleh kosong"));
		$f=strlen($password);
		if(($f<8)||($f>18))
			return (array(false,"karakter minimal adalah 8 dan maksimal 18"));
		if(!preg_match("/^[a-zA-Z0-9@]+$/",$password))
			return (array(false,"password terdiri dari huruf(besar,kecil), angka, karakter ('@')"));
		if(!strpos($password,"@")){
			return (array(false,"harus   mengandung '@'"));
		}
		if(!$this->isContainNumber($password)){
	...
	*/
	public function passwordFiltering($password){
		...
		if(!preg_match("/^[a-zA-Z0-9]+$/",$password))
			return (array(false,"password terdiri dari huruf(besar,kecil) dan angka"));
		if(!$this->isContainNumber($password)){
			return (array(false,"harus   mengandung angka"));
		}
	...
	*/
	...
	public function nipFiltering($variabel){
		if(!$this->numberFiltering($variabel)[0]) return array(false,"nip tidak valid");
		if(strlen($variabel) <= 15 || strlen($variabel) >= 21) return array(false,"nip tidak valid");
		return array(true,"nip valid");
	}
	public function noHandphoneFiltering($variabel){
		if(!$this->numberFiltering($variabel)[0]) return array(false,"nip tidak valid");
		if(strlen($variabel) <= 10 || strlen($variabel) >= 14) return array(false,"nip tidak valid");
		return array(true,"nip valid");
	}
	/*
	test : true
	...
	*/
	...
	public function textFiltering($variabel){
		if($this->isContainMoreSpace($variabel))
			return (array(false,"mengandung konten spasi berganda"));
		if(!preg_match("~^([a-zA-Z0-9]+)([a-zA-Z0-9 \&\\\"\'\,\-\.\:\(\)_]*)([a-zA-Z0-9 /\\\(\)\,\.\:\'\"\._]+)$~",$variabel))
			return (array(false,"mengandung karakter lain yang tidak diizinkan dalam judul"));
		return (array(true,"valid"));
	}
	public function nameSchoolFiltering($variabel){
		if($this->isContainMoreSpace($variabel))
			return (array(false,"mengandung konten spasi berganda"));
		if(!preg_match("/^([a-zA-Z0-9]+)([a-zA-Z0-9 -]*)([a-zA-Z0-9]+)$/",$variabel))
			return (array(false,"mengandung karakter lain yang tidak diizinkan dalam judul"));
